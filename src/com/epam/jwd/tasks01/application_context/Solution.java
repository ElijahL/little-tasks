...
public class Solution {
    ...
    public static double[][] tenthTask(double a, double b, double step) throws IllegalArgumentException {
        ...
        int numberOfArguments = (int) Math.round((b - a) / step);
        double[][] table = new double[numberOfArguments + 1][numberOfArguments + 1];
        for (int i = 0; i < numberOfArguments; ++i){
            table[i][0] = a + i * step;
        }
        table[numberOfArguments][0] = b;

        for (int i = 0; i < numberOfArguments + 1; ++i){
            table[i][1] = Math.tan(table[i][0]);
        }
        table[numberOfArguments][1] = Math.tan(table[numberOfArguments][0]);
